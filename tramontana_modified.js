...
var BrowserDetect = {
    ...
    searchString: function (e) {
        for (var t = 0; t < e.length; t++) {
            var s = e[t].string, n = e[t].prop;
            if (this.versionSearchString = e[t].versionSearch || e[t].identity, s) {
...
function get_appropriate_ws_url(e) {
    var t, s = document.URL;
    "https" == s.substring(0, 5) ? (t = "wss://", s = s.substr(8)) : (t = "ws://", "http" == s.substring(0, 4) && (s = s.substr(7))), s = s.split("/");
    return t + ("" == e ? s[0] : e)
}
...
String.prototype.replaceAll = function (e, t) {
    ...
    function e(t, s, n) {
        ...
        var i, c = this, d = !1, u = !1, l = document.createElement("div");
        l.addEventListener("open", function (e) {
            ...
            i = new WebSocket(c.url, s || []), t || l.dispatchEvent(r("connecting")), (c.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "attempt-connect", c.url);
            var n = i, a = setTimeout(function () {
                (c.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "connection-timeout", c.url), u = !0, n.close(), u = !1
            }, c.timeoutInterval);
            i.onopen = function () {
                clearTimeout(a), (c.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "onopen", c.url), c.protocol = i.protocol, c.readyState = WebSocket.OPEN, c.reconnectAttempts = 0;
                var s = r("open");
                s.isReconnect = t, t = !1, l.dispatchEvent(s)
            }, i.onclose = function (s) {
                if (clearTimeout(a), i = null, d) c.readyState = WebSocket.CLOSED, l.dispatchEvent(r("close")); else {
                    c.readyState = WebSocket.CONNECTING;
                    var n = r("connecting");
                    n.code = s.code, n.reason = s.reason, n.wasClean = s.wasClean, l.dispatchEvent(n), t || u || ((c.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "onclose", c.url), l.dispatchEvent(r("close")));
                    var a = c.reconnectInterval * Math.pow(c.reconnectDecay, c.reconnectAttempts);
                    setTimeout(function () {
                ...
                (c.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "onerror", c.url, t), l.dispatchEvent(r("error"))
            }
        }, 1 == this.automaticOpen && this.open(!1), this.send = function (t) {
            if (i) return (c.debug || e.debugAll) && console.debug("ReconnectingWebSocket", "send", c.url, t), i.send(t);
...
}), window.tramontanaMaster = {}, window.tramontanaMaster.state = "INIT", window.tramontanaMaster.changedStateInCycle = 0, tramontanaMaster.changeState = function (e) {
    e != tramontana.state && (tramontanaMaster.state = e, tramontanaMaster.changedStateInCycle = 1, jQuery.event.trigger("stateChanged", {}))
}, window.onbeforeunload = function () {
    for (var e in tramontanaMaster) ;
}, window.tramontana = function () {
    var e = new Object;
    return e.socket = new Object, e.status = "INIT", e.subscribed = {
        distance: 0,
        orientation: 0,
        attitude: 0,
        magnetometer: 0,
        ...
        if ("" != t) {
            -1 == t.indexOf(":") && (t += ":9092"), "Firefox" == BrowserDetect.browser ? console.error("Websocket not supported. Please use Safari or Chrome.") : e.socket = new ReconnectingWebSocket(get_appropriate_ws_url(t)), null != s && (e.callbacks.socketopen = s);
            try {
                e.socket.onopen = function () {
                    for (var t in e.status = "OPEN", jQuery.event.trigger("socketOpened", []), e.retrial = 0, e.subscribed) e.subscribed[t] && e.subscribeFunctions[t]();
        ...
        2 == arguments.length && this.sendMessage('{"m":"srv","n":' + e + ',"v":' + t + "}")
        ...
        2 == arguments.length && this.sendMessage('{"m":"rel","n":' + e + ',"v":' + Math.round(t) + "}")
    }, e.sendSerialMessageEmbedded = function (e) {
        1 == arguments.length && this.sendMessage('{"m":"tx","v":"' + e + '"}')
    }, e.setColorEmbedded = function (e, t, s, n) {
        this.sendMessage('{"m":"col","n":"' + e + '","r":"' + Math.floor(t) + '","g":"' + Math.floor(s) + '","b":"' + Math.floor(n) + '"}')
    }, e.blinkColorEmbedded = function (e, t, s, n) {
        4 == arguments.length && this.sendMessage('{"m":"blk","n":"' + e + '","r":"' + Math.floor(t) + '","g":"' + Math.floor(s) + '","b":"' + Math.floor(n) + '"}')
    }, e.setAllColorEmbedded = function (e, t, s) {
        3 == arguments.length && this.sendMessage('{"m":"all","r":"' + Math.floor(e) + '","g":"' + Math.floor(t) + '","b":"' + Math.floor(s) + '"}')
    }, e.setColor = function (e, t, s, n) {
        if (3 != arguments.length && 4 != arguments.length || ((e > 1 || t > 1 || s > 1) && (e /= 255, t /= 255, s /= 255, n /= 255), this.sendMessage('{"m":"setColor","r":"' + e + '","g":"' + t + '","b":"' + s + '","a":"' + n + '"}')), 1 == arguments.length && arguments[0].hasOwnProperty(r)) {
            var a = e;
            (a.r > 1 || a.g > 1 || a.b > 1) && (a.r = a.r / 255, a.g = a.g / 255, a.b = a.b / 255, a.a = a.a / 255), this.sendMessage('{"m":"setColor","r":"' + a.r + '","g":"' + a.g + '","b":"' + a.b + '","a":"' + a.a + '"}')
        }
    }, e.setFlashLight = function (e) {
        e = clamp(e, 0, e), this.sendMessage('{"m":"setLED","value":"1","in":"' + e + '"}')
    }, e.pulseFlashLight = function (e, t, s) {
        3 == arguments.length ? this.sendMessage('{"m":"pulseLED","t":"' + e + '","d":"' + t + '","i":"' + s + '"}') : console.warn("pulseLED requires 3 inputs: numberOfPulses,duration,intensity")
    }, e.showImage = function (e) {
        "string" == typeof e ? this.sendMessage('{"m":"showImage","url":"' + e + '"}') : console.warn("showImage requires a string input")
    }, e.playVideo = function (t, s) {
        e.callbacks.videoended = null != s ? s : 0, "string" == typeof t ? this.sendMessage('{"m":"playVideo","url":"' + t + '"}') : console.warn("playVideo requires a string input")
    }, e.loopVideo = function (t, s) {
        e.callbacks.videoended = null != s ? s : 0, "string" == typeof t ? this.sendMessage('{"m":"loopVideo","url":"' + t + '"}') : console.warn("loopVideo requires a string input")
    }, e.playAudio = function (e) {
        "string" == typeof e ? this.sendMessage('{"m":"playAudio","url":"' + e + '"}') : console.warn("playAudio requires a string input")
    }, e.setBrightness = function (e) {
        isNumber(e) ? this.sendMessage('{"m":"setBrightness","b":"' + e + '"}') : console.warn("setBrighness requires one numerical parameter")
    }, e.takePicture = function (e, t) {
        if (arguments.length > 0 && isNumber(e)) {
            var s = '{"m":"takePicture","c":"' + e + '"';
            arguments.length > 1 && "ui" == t && (s += ',"i":"ui"'), s += "}", this.sendMessage(s)
        } else this.sendMessage('{"m":"takePicture"}')
    }, e.transitionColors = function (e, t, s) {
        3 == arguments.length ? (isNumber(s) || (s = 5, console.warn("transitionColors: duration is not a number")), this.sendMessage('{"m":"transitionColors","r1":"' + e.r + '","g1":"' + e.b + '","b1":"' + e.g + '","a1":"' + e.a + '","r2":"' + t.r + '","g2":"' + t.g + '","b2":"' + t.b + '","a2":"' + t.a + '","duration":"' + s + '"}')) : console.warn("transitionColors requires 3 parameters: color1,color2,duration")
        ...
        e.subscribed.audiojack = 1, e.sendMessage('{"m":"registerAudioJack"}'), e.callbacks.audiojack = null != t ? t : 0
    }, e.releaseAudioJack = function () {
        e.subscribed.audiojack = 0, e.sendMessage('{"m":"releaseAudioJack"}')
    }, e.subscribeFunctions.powersource = e.subscribePowerSource = function (t) {
        e.subscribed.powersource = 1, e.sendMessage('{"m":"registerPowerSource"}'), e.callbacks.powersource = null != t ? t : 0
    }, e.releasePowerSource = function () {
        e.subscribed.powersource = 0, e.sendMessage('{"m":"releasePowerSource"}')
    }, e.subscribeFunctions.magnetometer = e.subscribeMagnetometer = function (t) {
        e.subscribed.magnetometer = 1, e.sendMessage('{"m":"registerMagnetometer"}'), e.callbacks.magnetometer = null != t ? t : 0
    }, e.releaseMagnetometer = function () {
        e.subscribed.magnetometer = 0, e.sendMessage('{"m":"releaseMagnetometer"}')
    }, e.subscribeFunctions.orientation = e.subscribeOrientation = function (t) {
        e.subscribed.orientation = 1;
        e.sendMessage('{"m":"registerOrientation"}'), e.callbacks.orientation = null != t ? t : 0
    }, e.releaseOrientation = function () {
        e.subscribed.orientation = 0, e.sendMessage('{"m":"releaseOrientation"}')
    }, e.subscribeFunctions.rxembedded = e.subscribeRxEmbedded = function (t) {
        e.subscribed.rxembedded = 1, e.sendMessage('{"m":"srx"}'), e.callbacks.rxembedded = null != t ? t : 0
    }, e.releaseRxEmbedded = function () {
        e.subscribed.rxembedded = 0, e.sendMessage('{"m":"drx"}')
    }, e.subscribeFunctions.ldrembedded = e.subscribeLDREmbedded = function (t, s) {
        var n = '{"m":"sldr","f":"' + t + '"}';
        e.subscribed.ldrembedded = 1, e.sendMessage(n), e.callbacks.ldrembedded = null != s ? s : 0
    }, e.releaseLDREmbedded = function () {
        e.subscribed.ldrembedded = 0, e.sendMessage('{"m":"dldr"}')
    }, e.subscribeFunctions.btnsembedded = e.subscribeButtonsEventEmbedded = function (t) {
        e.subscribed.btnsembedded = 1, e.sendMessage('{"m":"sbtn"}'), e.callbacks.btnsembedded = null != t ? t : 0
    }, e.releaseButtonsEventEmbedded = function () {
        e.subscribed.btnsembedded = 0, e.sendMessage('{"m":"dbtn"}')
    }, e.makeVibrate = function (t) {
        var s;
        s = "" != t && null != t && "" != t ? '{"m":"makeVibrate","duration":"' + t + '"}' : '{"m":"makeVibrate"}', e.sendMessage(s)
    }, e.sendMessage = function (t) {
        try {
            e.socket.send(t), e.retrial = 0
        } catch (s) {
            e.retrial < 3 ? (e.socket.refresh(), e.retrial++, setTimeout(function () {
                e.sendMessage(t)
            }, 100)) : console.warn("communication error: " + s)
        }
    }, e
};
